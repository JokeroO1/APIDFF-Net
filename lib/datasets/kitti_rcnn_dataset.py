...
class KittiRCNNDataset(KittiDataset):
    ...
    def get_lidar_pseudo(self, idx):
        ...
        lidar_pseudo_file = os.path.join(self.imageset_dir, 'depth_pseudo_rgbseguv_twise','%06d.bin' % idx)


        assert os.path.exists(lidar_pseudo_file)
        num_ponit_features_pseudo = 9
    ...
    def mask_points_and_boxes_outside_range_sfd(self, data_dict=None, config=None):
        ...
        #mask = self.mask_points_by_range(data_dict['points_pseudo'], [0, -40, -3, 70.4, 40, 1])

        limit_range = [0, -40, -3, 70.4, 40, 1]
    ...
    def grid_sample_points_pseudo(self, data_dict=None, config=None):
        ...
        data_dict['points_pseudo'] = points[choice, :]
        return data_dict
    ...
    def get_rpn_with_li_fusion(self, index):
        ...
        if sample_id < 10000:
            ...
            points_pseudo = self.get_lidar_pseudo(sample_id)
        ...
        sample_info = { 'sample_id'    : sample_id, 'random_select': self.random_select, 'img': img,
                        'pts_origin_xy': ret_pts_origin_xy , 'points_pseudo': points_pseudo}

        sample_info = self.mask_points_and_boxes_outside_range_sfd(sample_info)
        sample_info = self.grid_sample_points_pseudo(sample_info)
