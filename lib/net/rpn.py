...
class RPN(nn.Module):
    ...
    def forward(self, input_data):
        ...
        if cfg.LI_FUSION.ENABLED:
            ...
            # print("input_data['points_pseudo']",input_data['points_pseudo'].shape)
            #points_pseudo = input_data['points_pseudo']
            backbone_xyz, backbone_features = self.backbone_net(pts_input, img_input,xy_input)
            #backbone_xyz, backbone_features = self.backbone_net(pts_input, img_input, xy_input, points_pseudo)  # (B, N, 3), (B, C, N)
        else:
